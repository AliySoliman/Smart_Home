#include <WiFi.h>
#include <PubSubClient.h>
#include <WiFiClientSecure.h>
#include <ESP32Servo.h>

// WiFi credentials
char ssid[] = "WE_9BFF74";
char pass[] = "m2315798";

//---- HiveMQ Cloud Broker settings
const char* mqtt_server = "9e76f359454b4390a6aee8af043690d7.s1.eu.hivemq.cloud";
const char* mqtt_username = "Ali Soliman";
const char* mqtt_password = "Ali.Sief1521";
const int mqtt_port = 8883;

const int Living_LED = 12; // Pin connected to DOUT of flame sensor
const int Garage_LED = 4; // Pin connected to AOUT of gas sensor
const int Kitchen_LED = 27; // Pin connected to buzzer

Servo Main_Door;
Servo Garage_Door;
Servo Window;

WiFiClientSecure espClient;
PubSubClient client(espClient);

// HiveMQ Cloud Let's Encrypt CA certificate
static const char *root_ca PROGMEM = R"EOF(
-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
)EOF";

unsigned long lastMsg = 0;
#define MSG_BUFFER_SIZE (500)
char msg[MSG_BUFFER_SIZE];
int value = 0;

void setup_wifi() {
delay(10);
// We start by connecting to a WiFi network
Serial.println();
Serial.print("Connecting to ");
Serial.println(ssid);

WiFi.mode(WIFI_STA);
WiFi.begin(ssid, pass);

while (WiFi.status() != WL_CONNECTED) {
delay(500);
Serial.print(".");
}

randomSeed(micros());

Serial.println("");
Serial.println("WiFi connected");
Serial.println("IP address: ");
Serial.println(WiFi.localIP());
}

void callback(char* topic, byte* payload, unsigned int length) {
    Serial.print("Message arrived [");
    Serial.print(topic);
    Serial.print("] ");
    String arrived_mess;
    for (int i = 0; i < length; i++) {
        arrived_mess += (char)payload[i];
    }

    if (strcmp(topic, "Home/Main_Door") == 0 || 
    strcmp(topic, "Home/Garage_Door") == 0 || 
    strcmp(topic, "Home/Window") == 0 ||
    strcmp(topic, "LED/Living") == 0 || 
    strcmp(topic, "LED/Garage") == 0 || 
    strcmp(topic, "LED/Kitchen") == 0) {

    switch (topic[5]) { // using 5th character to differentiate between categories
        case 'M': // Main_Door
            if (strcmp(topic, "Home/Main_Door") == 0) {
                if (arrived_mess.equals("open")) {
                    Main_Door.write(90);
                } else if (arrived_mess.equals("close")) {
                    Main_Door.write(170);
                }
            }
            break;

        case 'G': // Garage_Door and LED/Garage
            if (strcmp(topic, "Home/Garage_Door") == 0) {
                if (arrived_mess.equals("open")) {
                    Garage_Door.write(90);
                } else if (arrived_mess.equals("close")) {
                    Garage_Door.write(50);
                }
            } else if (strcmp(topic, "LED/Garage") == 0) {
                if (arrived_mess.equals("on")) {
                    digitalWrite(Garage_LED, HIGH);
                } else if (arrived_mess.equals("off")) {
                    digitalWrite(Garage_LED, LOW);
                }
            }
            break;

        case 'W': // Window
            if (strcmp(topic, "Home/Window") == 0) {
                if (arrived_mess.equals("open")) {
                    Window.write(150);
                } else if (arrived_mess.equals("close")) {
                    Window.write(60);
                }
            }
            break;

        default : // LED/Living, LED/Garage, LED/Kitchen
            switch (topic[4]) { // using 4th character to differentiate LED locations
                case 'L': // LED/Living
                    if (strcmp(topic, "LED/Living") == 0) {
                        if (arrived_mess.equals("on")) {
                            digitalWrite(Living_LED, HIGH);
                        } else if (arrived_mess.equals("off")) {
                            digitalWrite(Living_LED, LOW);
                        }
                    }
                    break;
                case 'G': // LED/Garage
                    if (strcmp(topic, "LED/Garage") == 0) {
                        if (arrived_mess.equals("on")) {
                            digitalWrite(Garage_LED, HIGH);
                        } else if (arrived_mess.equals("off")) {
                            digitalWrite(Garage_LED, LOW);
                        }
                    }
                    break;
                case 'K': // LED/Kitchen
                    if (strcmp(topic, "LED/Kitchen") == 0) {
                        if (arrived_mess.equals("on")) {
                            digitalWrite(Kitchen_LED, HIGH);
                        } else if (arrived_mess.equals("off")) {
                            digitalWrite(Kitchen_LED, LOW);
                        }
                    }
                    break;
            }
            break;
    }
}

}

void reconnect() {
// Loop until we’re reconnected
while (!client.connected()) {
Serial.print("Attempting MQTT connection… ");
String clientId = "ESP32Client";
// Attempt to connect
if (client.connect(clientId.c_str(), mqtt_username, mqtt_password)) {
Serial.println("connected!");
// Once connected, publish an announcement...
client.publish("ESP32/status", "connected");
// ... and resubscribe
client.subscribe("Home/Main_Door");
client.subscribe("Home/Garage_Door");
client.subscribe("Home/Window");
client.subscribe("LED/Garage");
client.subscribe("LED/Kitchen");
client.subscribe("LED/Living");

} else {
Serial.print("failed, rc = ");
Serial.print(client.state());
Serial.println(" try again in 5 seconds");
// Wait 5 seconds before retrying
delay(5000);
}
}
}


void setup() {
delay(500);
// When opening the Serial Monitor, select 9600 Baud
Serial.begin(9600);

pinMode(Living_LED,OUTPUT);
pinMode(Garage_LED,OUTPUT);
pinMode(Kitchen_LED,OUTPUT);

Main_Door.attach(13);
Garage_Door.attach(14);
Window.attach(15);

Main_Door.write(170);
Garage_Door.write(50);
Window.write(60);

delay(500);
setup_wifi();
espClient.setCACert(root_ca);
client.setServer(mqtt_server, mqtt_port);
client.setCallback(callback);
}

void loop() {
if (!client.connected()) {
reconnect();
}
client.loop();

}